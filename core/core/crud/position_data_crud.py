...
from core.orm_models.target_positions import TargetPositions

logging = logger(__name__)


class CRUDPositionData:
    def create(self, **kwargs):
        ...
        try:
            ...
            sensor_data = TargetPositions(**kwargs)
    ...
    def read(self, position_id: str):
        ...
        try:
            ...
            with session() as transaction_session:
                obj: TargetPositions = (
                    transaction_session.query(TargetPositions)
                    .filter(TargetPositions.position_id == position_id)
                    .first()
                )
            if obj is not None:
                return obj.__dict__
    ...
    def read_by_data(
        ...
        try:
            ...
            with session() as transaction_session:
                obj: TargetPositions = (
                    transaction_session.query(TargetPositions)
                    .filter(TargetPositions.water_temperature == water_temp)
                    .filter(TargetPositions.ph_level == ph_level)
                    .filter(TargetPositions.tds_level == tds_level)
                    .first()
                )
            if obj is not None:
                return obj.__dict__
    ...
    def read_all(self):
        ...
        try:
            ...
            with session() as transaction_session:
                obj: TargetPositions = transaction_session.query(TargetPositions).all()
            if obj is not None:
